...
header_html = f"""
    <header>
        <h2 style="text-align: center; margin-top:10px;"> <img src='data:image/png;base64,{image_bytes}' alt="My Logo" style="float: left; margin-bottom: 10px;width:100px;height:100px;"> SEBI PORTAL</h1>
    </header>
    """
st.markdown(
...
if choice == "CLAIM DETECTION":
    ...
    if selected == "AUDIO":
        ...
        if audio_file:
            ...
            source_type = st.text_area("Source Type")
            if st.button("Submit") and prompt:
                response = new_claim_detection_pipeline(prompt)
                add_details_source(source_name,source_type,response)
                financial_points = financial_point_extractor(prompt)
                display_sebi_rules(financial_points)
